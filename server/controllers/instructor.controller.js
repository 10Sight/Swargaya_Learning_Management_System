...
export const getDashboardStats = asyncHandler(async (req, res) => {
  ...
  const [departments] = await pool.query("SELECT * FROM departments WHERE instructor = ?", [instructorId]);
  ...
  if (courseIds.length > 0) {
    const [cRows] = await pool.query("SELECT COUNT(*) as count FROM courses WHERE id IN (?) AND status = 'PUBLISHED'", [courseIds]);
  ...
  if (allStudentIds.length > 0) {
    const [attempts] = await pool.query(`
            ...
            WHERE aq.student IN (?)
            ORDER BY aq.createdAt DESC LIMIT 10
        `, [allStudentIds]);
    ...
    const [submissions] = await pool.query(`
            ...
            WHERE s.student IN (?) 
            ORDER BY s.submittedAt DESC LIMIT 10
        `, [allStudentIds]);
...
export const getAssignedCourses = asyncHandler(async (req, res) => {
  ...
  const [departments] = await pool.query("SELECT course FROM departments WHERE instructor = ?", [instructorId]);
  const courseIds = [...new Set(departments.map(d => d.course).filter(Boolean))];
  ...
  const [countRows] = await pool.query(`SELECT COUNT(*) as total FROM courses WHERE id IN (?) AND status = 'PUBLISHED' ` + (search ? "AND (title LIKE ? OR description LIKE ? OR category LIKE ?)" : ""), params);
...
export const getCourseDetails = asyncHandler(async (req, res) => {
  ...
  const [depts] = await pool.query("SELECT id FROM departments WHERE instructor = ? AND course = ?", [instructorId, courseId]);
...
export const getAssignedDepartments = asyncHandler(async (req, res) => {
  ...
  const departmentsWithProgress = await Promise.all(departments.map(async (d) => {
    if (d.course) {
      const [c] = await pool.query("SELECT id, title, category, difficulty FROM courses WHERE id = ?", [d.course]);
    ...
    if (studentIds.length > 0) {
      const [sRows] = await pool.query("SELECT id, fullName, email, userName, status, avatar FROM users WHERE id IN (?)", [studentIds]);
      students = sRows;

      if (d.course) {
        const [progRows] = await pool.query("SELECT student, progressPercent FROM progress WHERE course = ? AND student IN (?)", [d.course.id, studentIds]);
...
export const getDepartmentDetails = asyncHandler(async (req, res) => {
  ...
  const [rows] = await pool.query("SELECT * FROM departments WHERE id = ? AND instructor = ?", [departmentId, instructorId]);
  ...
  if (studentIds.length > 0) {
    const [s] = await pool.query("SELECT id, fullName, email, phoneNumber FROM users WHERE id IN (?)", [studentIds]);
...
export const getDepartmentStudents = asyncHandler(async (req, res) => {
  ...
  const [dRows] = await pool.query("SELECT * FROM departments WHERE id = ? AND instructor = ?", [departmentId, instructorId]);
  ...
  const [students] = await pool.query("SELECT id, fullName, email, slug, phoneNumber, userName, status, avatar FROM users WHERE id IN (?)", [studentIds]);

  let progressMap = {};
  if (department.course) {
    const [pRows] = await pool.query("SELECT student, progressPercent FROM progress WHERE course = ? AND student IN (?)", [department.course, studentIds]);
...
export const getStudentProgress = asyncHandler(async (req, res) => {
  ...
  const [myDepts] = await pool.query("SELECT students, course FROM departments WHERE instructor = ?", [instructorId]);
  ...
  if (progress) {
    ...
    if (completedIds.length > 0) {
      const [lessons] = await pool.query("SELECT title FROM lessons WHERE id IN (?)", [completedIds]);
...
export const getDepartmentQuizAttempts = asyncHandler(async (req, res) => {
  ...
  sql += " ORDER BY aq.createdAt DESC LIMIT ? OFFSET ?";
  params.push(parseInt(limit), offset);
...
export const getDepartmentAssignmentSubmissions = asyncHandler(async (req, res) => {
  ...
  sql += " ORDER BY s.submittedAt DESC LIMIT ? OFFSET ?";
  params.push(parseInt(limit), offset);
